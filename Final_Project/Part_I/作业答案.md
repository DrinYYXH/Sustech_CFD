...
**方法设计：** 使用光滑初值 $\sin(\pi x)$（消除 LW 自身振荡对检测的干扰），$N=80$，在五个 CFL 值（0.5, 0.8, 1.0, 1.05, 1.1）下运行至 $t=10.0$。跟踪 **$L_1$ 范数**（网格平均绝对解）：

$$L_1(t) = \frac{1}{N}\sum_{i=1}^{N}|u_i(t)|$$

对于精确解 $\sin(\pi(x-t))$，$L_1$ 严格守恒：$L_1^{\text{exact}} = 2/\pi \approx 0.6366$。若数值解稳定，$L_1$ 在 $0.637$ 附近微小波动；若 $L_1$ 开始偏离此参考值并指数增长，则判定为失稳。具体判据：$L_1 > 10^{10}$ 或出现 NaN。

**为何使用 $L_1$ 范数而非 $\max|u|$？**

| 指标 | $\max|u|$ | $L_1 = \frac{1}{N}\sum|u_i|$ |
|------|:---:|:---:|
| 对单点异常的敏感度 | 高——一个网格溢出即触达 $10^{10}$ | 低——单点异常被 $1/N$ 因子稀释 |
| 物理参考值 | 无守恒量（$\max|\sin| = 1$） | 守恒 $2/\pi \approx 0.637$ |
| 统计稳定性 | 受单点舍入误差波动影响大 | 网格平均，噪声平滑 |
| 对不稳定波数分量的灵敏度 | 不区分波数 | 所有波数均贡献——哪个波数先发散都能检测 |
| 早期偏离检测 | 拐点可能模糊 | 拐点更清晰（从常数 0.637 开始偏离） |

**$L_1$ 的核心优势：** 提供了一个不随时间变化的精确参考值 $2/\pi$。任何偏离此常数的趋势都是不稳定性的早期信号，不需要等 $\max|u|$ 达到 $10^{10}$ 才做判断。
...
> - 失稳判据：第 51 行（`l1norm > blowup .or. l1norm /= l1norm`），其中 `blowup = 1.0d10`（第 20 行），`l1norm /= l1norm` 检测 NaN
> - 采样间隔：第 39 行（`output_every = max(1, nsteps / 200)`），每 CFL 约 200 个采样点
> - LW 扫描循环：第 45–59 行；VL 扫描循环：第 62–76 行；SB 扫描循环：第 79–93 行
...
表格中的发散时间（"t≈8.6 发散"）是用 $L_1 > 10^{10}$ 或 NaN 作为判据得到的。这个阈值存在两个问题：

**(1) 阈值过高，混淆了拐点与斜率两个独立信息。** 从稳定性图上可以观察到——LW 的曲线在更早的时间点就已开始偏离 $L_1 \approx 0.637$（拐点更早），而 VL/SB 的偏离起始更晚。但由于 VL/SB 在后段的直线斜率**更陡**（增长率更高），最终三者到达 $10^{10}$ 的时间几乎相近。单一阈值 $10^{10}$ 将拐点时间和斜率这两个独立的物理量压缩为一个标量，丢失了全部动力学信息。

**(2) 在到达 $10^{10}$ 之前，解早已失去物理意义。** $L_1$ 偏离 $0.637$ 超过 10%（即 $L_1 > 0.7$）时数值解已不可用——此后的数据仅具有稳定性分析的诊断价值。用 $10^{10}$ 作为"发散时刻"给出的是格式完全崩溃的时间点，而非可用性的终止点。**$L_1$ 的优势在于提供了明确的"可用性终止"参考线（$L_1 = 0.637$ 的偏离），这是 $\max|u|$ 无法提供的。**

正确的分析应分别报告：**(a) 偏离起始时间（拐点，如 $L_1$ 首次偏离 $2/\pi$ 超过 10% 的时刻）** 和 **(b) 指数增长段的斜率 $\alpha$**。
...
在半对数坐标中，$L_1 \propto |G|_{\max}^{t/\Delta t}$。取对数：

$$\log_{10}(L_1) = \frac{t}{\Delta t} \log_{10}|G|_{\max} + \text{const}$$
